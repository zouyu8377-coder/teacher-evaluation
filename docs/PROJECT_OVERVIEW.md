...
|------|-----|
| 项目名称 | 教师评价考核平台 |
| 当前版本 | v1.4.3（前端 package.json / 后端 pom.xml / 部署镜像均已同步） |
...
平台围绕教师职称/级别晋升考核，支持 **C → B → A** 三级晋升体系；B2/B1 是 B 级下的并列活动类型，A2/A1 是 A 级下的并列活动类型。
...
| 版本 | 主要变更 |
|------|---------|
| v1.4.3 | 增加用户 Excel 模板下载、批量导入更新、筛选导出；优化教师活动可见性与发布成绩截止逻辑 |
| v1.4.2 | 统一考核状态机，收紧权限边界，生产初始化治理，前端类型检查与依赖安全更新 |
| v1.3.0 | 引入教师等级体系：持久化等级、自动晋升、历史记录、管理端等级修改 |
| v1.2.x | 考核功能完善、成绩发布优化、C级考试修复 |
| v1.1.7 | 前后端类型安全重构 |
